...
GOLDEN = BASE / 'insight_prospecting_bg_golden_30.jsonl'
RAW = BASE / 'insight_prospecting_bg_raw_traces.json'
OUT_ALL = BASE / 'insight_prospecting_bg_golden_30_rows.json'
OUT_DIR = BASE / 'insight_prospecting_bg_golden_30_rows'
...
def main() -> int:
    ...
    if not isinstance(raw_rows, list):
        raise SystemExit('Raw traces file must contain data[] list')

    by_id = {
        str(r.get('id') or r.get('root_span_id') or ''): r
        for r in raw_rows
    ...
    for g in golden:
        ...
        if raw_row is None:
            raise SystemExit(f'Missing raw trace row for trace_id: {tid}')
        ...
        per_row_path = OUT_DIR / f'row_{tid}.json'
        per_row_path.write_text(json.dumps([row_obj], ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
