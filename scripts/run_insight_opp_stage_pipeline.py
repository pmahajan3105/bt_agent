...
def _count_query_by_prompt_name(project_id: str, slug: str, hours: int) -> str:
    return (
        "select: count(*) as n\n"
        f"from: project_logs('{project_id}')\n"
        "spans\n"
        ...
        f"created > now() - interval {hours} hour and "
        "created < now() and "
        f"metadata.prompt_name = '{slug}' and "
        "span_attributes.parent_span_id is null\n"
    )
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Run full insight opp-stage golden pipeline")
    ...
    prompt_name_count_72 = _extract_count(
        bt.btql(_count_query_by_prompt_name(args.project_id, args.prompt_slug, 72))
    )
    ...
    for hours in windows:
        ...
        query_path = queries_dir / f"insight_opp_stage_bg_{hours}h.btql"
        ...
        raw_path = out_dir / "insight_opp_stage_bg_raw_traces.json"
        ...
        # Save iterative artifacts for traceability.
        write_jsonl(out_dir / "insight_opp_stage_bg_candidates_structured.jsonl", candidates)
        write_jsonl(out_dir / "insight_opp_stage_bg_rejected_rows.jsonl", rejects)
        write_jsonl(out_dir / "insight_opp_stage_bg_judgements_v1.jsonl", judged)
        write_json(out_dir / "insight_opp_stage_bg_judgements_v1_stats.json", {
            "candidate_count": len(candidates),
            "label_counts": dict(Counter(str(r.get("judge_outcome")) for r in judged)),
            "critical_violation_counts": dict(Counter(v for r in judged for v in (r.get("critical_violations") or []))),
        })
    ...
    golden_path = out_dir / "insight_opp_stage_bg_golden_30.jsonl"
    renamed_path = out_dir / "insight_opp_stage_bg_golden_30_renamed.jsonl"
    labels_path = out_dir / "insight_opp_stage_bg_golden_30_labels_only.jsonl"
    curation_log_path = out_dir / "insight_opp_stage_bg_curation_log.jsonl"
    ...
    rejected_without_reason = [
        ...
        and not isinstance(log.get("reject_reasons"), list)
        or (
            str(log.get("worthy_decision")) == "rejected"
            and isinstance(log.get("reject_reasons"), list)
            and len(log.get("reject_reasons")) == 0
        )
    ]
    ...
    report_lines = [
        "# Insight Opportunity Stage Golden Dataset Report",
        "",
        "## Query Correctness",
        f"- Prompt slug: `{args.prompt_slug}`",
        f"- Project id: `{args.project_id}`",
        f"- Count last 72h using `metadata.prompt_slug`: **{slug_count_72}**",
        f"- Count last 72h using `metadata.prompt_name` (same slug string): **{prompt_name_count_72}**",
        "- Result: slug-based filtering is required for this prompt's logs.",
        "",
        "## Pipeline Results",
        ...
        f"- `{labels_path}`",
        f"- `{curation_log_path}`",
        f"- `{out_dir / 'insight_opp_stage_bg_judgements_v1.jsonl'}`",
        f"- `{out_dir / 'insight_opp_stage_bg_judgements_v1_stats.json'}`",
        "",
        "## Selection Summary",
    ...
    report_path = out_dir / "insight_opp_stage_bg_golden_30_report.md"
