...
from insight_new_stage_pipeline_lib import (
    BraintrustClient,
    PROJECT_ID,
    PROMPT_SLUG,
    build_slug_query,
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Run full insight new-stage golden pipeline")
    ap.add_argument("--project-id", default=PROJECT_ID)
    ap.add_argument("--prompt-slug", default=PROMPT_SLUG)
    ap.add_argument("--prompt-name", default="account-overview-new-summarizer-background")
    ...
    ap.add_argument("--sample-per-window", type=int, default=40)
    args = ap.parse_args()
    ...
    for hours in windows:
        ...
        query_path = queries_dir / f"insight_new_stage_bg_{hours}h.btql"
        ...
        raw_path = out_dir / "insight_new_stage_bg_raw_traces.json"
        ...
        write_jsonl(out_dir / "insight_new_stage_bg_candidates_structured.jsonl", candidates)
        write_jsonl(out_dir / "insight_new_stage_bg_rejected_rows.jsonl", rejects)
        write_jsonl(out_dir / "insight_new_stage_bg_judgements_v1.jsonl", judged)
        write_json(
            out_dir / "insight_new_stage_bg_judgements_v1_stats.json",
            {
    ...
    golden_path = out_dir / "insight_new_stage_bg_golden_30.jsonl"
    renamed_path = out_dir / "insight_new_stage_bg_golden_30_renamed.jsonl"
    labels_path = out_dir / "insight_new_stage_bg_golden_30_labels_only.jsonl"
    curation_log_path = out_dir / "insight_new_stage_bg_curation_log.jsonl"
    ...
    report_lines = [
        "# Insight New Stage Golden Dataset Report",
        "",
        "## Query Correctness",
        ...
        "- Result: slug and prompt-name both checked; slug used for fetch consistency.",
        "",
        "## Pipeline Results",
        ...
        f"- `{labels_path}`",
        f"- `{curation_log_path}`",
        f"- `{out_dir / 'insight_new_stage_bg_judgements_v1.jsonl'}`",
        f"- `{out_dir / 'insight_new_stage_bg_judgements_v1_stats.json'}`",
        "",
        "## Selection Summary",
    ...
    report_path = out_dir / "insight_new_stage_bg_golden_30_report.md"
